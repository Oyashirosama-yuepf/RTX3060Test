...
{
public:
    Spline2dKernel(const std::vector<float64_t>& t_knots, const uint32_t spline_order);

    // customized input output
    void   AddRegularization(const float64_t regularization_param);
    bool_t AddKernel(const Eigen::MatrixXd& kernel, const Eigen::MatrixXd& offset, const float64_t weight);
    bool_t AddKernel(const Eigen::MatrixXd& kernel, const float64_t weight);
    ...
    bool_t AddReferenceLineKernelMatrix(const std::vector<float64_t>&        t_coord,
                                        const std::vector<geometry::Point2>& ref_points,
