 ...
 * @date "2019-09-17"
 */
#ifndef HOLO_PERCEPTION_LIB_VISION_FREESPACE_TIDL_H_
#define HOLO_PERCEPTION_LIB_VISION_FREESPACE_TIDL_H_
...
/**
 * @brief define VisionFreespaceTidl class
 */
// class VisionFreespaceTidl : public Worker
class VisionFreespaceTidl
{
public:
...
public:
     ...
     * message).
     */
    VisionFreespaceTidl(const std::string cfg_file, float hz = 10, int debug_2d_image = 0,
    ...
    /**
     * @brief check all image is received.
     */
    ...
    std::unordered_map<std::string, std::vector<holo::float32_t>> camera_resolutions_;  // will update on the arrival of
                                                                                        // images
    std::unordered_map<std::string, std::shared_ptr<SafeMsgDataContainer<cv::Mat>>> cameras_msg_data_container_;
    std::unordered_map<std::string, holo::Image>                                    cameras_raw_image_container_;
#ifdef TIDL_INFERENCE
    uint8_t*                                           data;
