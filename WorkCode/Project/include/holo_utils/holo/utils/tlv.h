...
public:
    ...
    V GetValue() const
    {
        ...
        {
            holo::exception::OutOfRangeException e{""};
            throw e;
        }
        return v;
    }
...
protected:
    ...
    void write(uint8_t* p, V value) noexcept
    {
        value = holo::core::hton16(value);
        (void)std::memcpy(p, &value, sizeof(V));
    }
    ...
    void read(uint8_t* p, V& value) const noexcept
    {
        (void)std::memcpy(&value, p, sizeof(V));
        value = holo::core::ntoh16(value);
    }
    ...
    size_t assignValues(size_t offset, T1&& v1, Args&&... args) noexcept
    {
        ...
        return assignValues(holo::serialization::AlignedSize<align_size>(offset) + sizeof(T1),
    ...
    {
        (void)std::copy(v1.cbegin(), v1.cend(), value_ + holo::serialization::AlignedSize<ALIGNMENT_VALUE>(offset));
    ...
    size_t assignValues(size_t offset, char* const& v1, Args&&... args) noexcept
    {
        ...
        return assignValues(holo::serialization::AlignedSize<ALIGNMENT_VALUE>(offset) + i + 1U,
                            std::forward<Args>(args)...);
    }
    ...
    constexpr size_t extractValues(size_t offset, T1&& v1, Args&&... args) const noexcept
    {
        ...
        return extractValues(holo::serialization::AlignedSize<align_size>(offset) + sizeof(T1),
                             std::forward<Args>(args)...);
    }
    ...
    constexpr size_t extractValues(size_t offset, std::string& v1, Args&&... args) const noexcept
    {
        ...
        v1.resize(size);
        (void)std::copy(value_ + start, value_ + start + size, v1.begin());
    ...
    {
        size_t const align_size = sizeof(T1) > ALIGNMENT_VALUE ? ALIGNMENT_VALUE : sizeof(T1);
        return getValueSize(holo::serialization::AlignedSize<align_size>(offset) + sizeof(T1),
                            std::forward<Args>(args)...);
    }
...
public:
    ...
    {
        size_t     head_offset{deserializer.GetOffset()};
        TypeType   type;
        LengthType length;
        ...
        if (base_ == nullptr)
        {
            base_ = const_cast<uint8_t*>(deserializer.GetData() + head_offset);
            size_ = deserializer.GetOffset() - head_offset;
        }
        else
        {
            ...
            std::memcpy(base_, deserializer.GetData() + head_offset, deserializer.GetOffset() - head_offset);
        }
