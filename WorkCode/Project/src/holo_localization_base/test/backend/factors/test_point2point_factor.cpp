...
/**
 * @file test_point2point_factor.cpp
 * @brief This header file defines test for point2point factor class
...
#include <holo/localization/sam/factors/point2point_factor.h>
#include <holo/log/hololog.h>

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
TEST(Point2PointFactor, Jacobian)
{
    /// Step 1: Prepare the input variables
    gtsam::Rot3 const   wRb = gtsam::Rot3::RzRyRx(0.2, -0.1, 0.3);
    gtsam::Point3 const wtb = gtsam::Point3(0.5, 0.25, -0.35);
    gtsam::Pose3 const  wTb(wRb, wtb);

    gtsam::Point3 const pt_b(0.5, 0.5, 0.5);

    gtsam::Point3 pt_w = wTb.transform_to(pt_b);

    /// Construct the key
    uint64_t const   pose_index = 1u;
    gtsam::Key const x1         = gtsam::symbol_shorthand::X(pose_index);

    /// Step 2: Create the Point factor
    auto                                    noise = gtsam::noiseModel::Isotropic::Sigma(1, 1e-3);
    holo::localization::GTPoint2PointFactor factor(x1, pt_b, pt_w, noise);
    ...
    ASSERT_TRUE(gtsam::internal::testFactorJacobians("Point2PointFactorJacobian", factor, values, 1e-5, 1e-5));
}

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
TEST(Point2PointFactor, Optimize)
{
    using Point2PointFactor = holo::localization::GTPoint2PointFactor;

    /// Step 1: Create the ground truth
    gtsam::Point3 const gt_position(0.25, 0.15, -0.35);
    gtsam::Rot3 const   gt_rotation = gtsam::Rot3::RzRyRx(0.1, -0.1, 0.15);
    ...
    auto point_noise = gtsam::noiseModel::Isotropic::Sigma(1, 1e-3);
    ...
    for (size_t i = 0; i < target_pts.size(); ++i)
    {
        gtsam::NoiseModelFactor::shared_ptr factor(
            new Point2PointFactor(x1, source_pts[i], target_pts[i], point_noise));
