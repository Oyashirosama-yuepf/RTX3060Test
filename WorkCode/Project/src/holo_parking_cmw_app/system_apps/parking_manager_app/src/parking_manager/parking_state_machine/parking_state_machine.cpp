...
void ParkStateMachineManager::Manage(ParkingInput const& parking_input) {
    ...
    // 刷新输出信息
    apa_state_ = ParkTinyFsm::GetParkingOutput().apa_state;
    apa_park_command_ = ParkTinyFsm::GetParkingOutput().apa_park_command;
    apa_ctrl_req_ = ParkTinyFsm::GetParkingOutput().apa_ctrl_req;
    route_id_ ++;
}
