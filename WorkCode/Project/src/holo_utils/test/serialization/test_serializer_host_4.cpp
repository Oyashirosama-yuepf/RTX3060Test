...
TEST(Serializer, serialize_char)
{
    ...
    {
        ser << expected_buf[i];
        ASSERT_EQ(expected_buf[i], static_cast<holo::char_t>(buffer[i]));
    }
    ser << holo::serialization::align;
...
TEST(Serializer, serialize_char_unaligned)
{
    ...
    {
        ser << expected_buf[i];
        ASSERT_EQ(expected_buf[i], static_cast<holo::char_t>(buffer[i + 1]));
    }
    ser << holo::serialization::align;
...
TEST(Serializer, serialize_i64_unaligned)
{
    ...
    for(uint8_t i = 0; i < 32; ++i)
    {
        ...
        ASSERT_EQ(((static_cast<uint64_t>(value)) & 0x00000000000000FF),       buffer[i * 8 + 4]);
        ASSERT_EQ(((static_cast<uint64_t>(value)) & 0x000000000000FF00) >> 8,  buffer[i * 8 + 1 + 4]);
        ASSERT_EQ(((static_cast<uint64_t>(value)) & 0x0000000000FF0000) >> 16, buffer[i * 8 + 2 + 4]);
        ASSERT_EQ(((static_cast<uint64_t>(value)) & 0x00000000FF000000) >> 24, buffer[i * 8 + 3 + 4]);
        ASSERT_EQ(((static_cast<uint64_t>(value)) & 0x000000FF00000000) >> 32, buffer[i * 8 + 4 + 4]);
        ASSERT_EQ(((static_cast<uint64_t>(value)) & 0x0000FF0000000000) >> 40, buffer[i * 8 + 5 + 4]);
        ASSERT_EQ(((static_cast<uint64_t>(value)) & 0x00FF000000000000) >> 48, buffer[i * 8 + 6 + 4]);
        ASSERT_EQ(((static_cast<uint64_t>(value)) & 0xFF00000000000000) >> 56, buffer[i * 8 + 7 + 4]); 
    }
    ser << holo::serialization::align;
...
TEST(Serializer, serialize_u64_unaligned)
{
    ...
    for(uint8_t i = 0; i < 32; ++i)
    {
        ...
        ASSERT_EQ((value & 0x00000000000000FF),       buffer[i * 8 + 4]);
        ASSERT_EQ((value & 0x000000000000FF00) >> 8,  buffer[i * 8 + 1 + 4]);
        ASSERT_EQ((value & 0x0000000000FF0000) >> 16, buffer[i * 8 + 2 + 4]);
        ASSERT_EQ((value & 0x00000000FF000000) >> 24, buffer[i * 8 + 3 + 4]);
        ASSERT_EQ((value & 0x000000FF00000000) >> 32, buffer[i * 8 + 4 + 4]);
        ASSERT_EQ((value & 0x0000FF0000000000) >> 40, buffer[i * 8 + 5 + 4]);
        ASSERT_EQ((value & 0x00FF000000000000) >> 48, buffer[i * 8 + 6 + 4]);
        ASSERT_EQ((value & 0xFF00000000000000) >> 56, buffer[i * 8 + 7 + 4]); 
    }
    ser << holo::serialization::align;
    ASSERT_EQ(260, ser.GetSize());
...
TEST(Serializer, serialize_f64_unaligned)
{
    ...
    for(uint8_t i = 0; i < 32; ++i)
    {
        ...
        ASSERT_EQ(value, *(reinterpret_cast<holo::float64_t const*>(ser.GetData() + 8 * i + 4)));
        ASSERT_EQ(value, *(reinterpret_cast<holo::float64_t const*>(buffer + 8 * i + 4)));
    }
    ser << holo::serialization::align;
    ASSERT_EQ(260, ser.GetSize());
...
TEST(Serializer, serialize_pre_align_custom_object)
{
    uint8_t    buffer[1024] = {0};
    Serializer ser(buffer, 1024);
    ...
    ASSERT_EQ(0x78, buffer[4]);
    ASSERT_EQ(0x56, buffer[5]);
    ASSERT_EQ(0x34, buffer[6]);
    ASSERT_EQ(0x12, buffer[7]);
    ASSERT_EQ(0x21, buffer[8]);
    ASSERT_EQ(0x43, buffer[9]);
    ASSERT_EQ(0x65, buffer[10]);
    ASSERT_EQ(0x87, buffer[11]);
...
TEST(Serializer, serialize_test_hybrid_object)
{
    ...
    uint8_t expected_buf[] ={
        0x01, 0x00, 0x01, 0x02, 0x01, 0x02, 0x03, 0x04,
        0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
        0x00, 0x00, 0x80, 0x3f, 0x00, 0x00, 0x00, 0x00, 
        0x00, 0x00, 0xf0, 0x3f, 0x04, 0x03, 0x00, 0x00, 
        0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 
