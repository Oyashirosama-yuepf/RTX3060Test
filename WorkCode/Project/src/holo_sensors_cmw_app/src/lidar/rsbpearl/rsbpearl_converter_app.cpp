...
/**
 * @file rsbpearl_converter_app.cpp
 * @brief A converter to convert rsbpearl lidar scan messages to pcd
 * @author wujie<wujie@holomatic.com>
 * @date 2022/01/12
 */

#include <lidar/rsbpearl/rsbpearl_converter_app.h>
#include <csignal>

namespace holo_cmw
{
void RsbpearlConverterApp::Setup(const std::string& config_file, const std::string out_fmt)
{
    ...
    if (!GetParameters(config_file))
    {
        ...
        oss << "rsbpearl lidar converter app failed to load config file";
        LOG(ERROR) << oss.str();
    ...
    try
    {
        ...
        if (publish_scan_)
        {
            ...
            if (extrinsic_path_ == "")
            {
                h0 = std::bind(&RsbpearlConverterApp::DataCallbackUseDefaultExtrinsic, this, std::placeholders::_1,
                               std::placeholders::_2);
                LOG(INFO) << "use default extrinsics from LidarScan";
            }
            else
            {
                h0 = std::bind(&RsbpearlConverterApp::DataCallbackUseUserExtrinsic, this, std::placeholders::_1,
                               std::placeholders::_2);
        ...
        else
        {
            ...
            h0 = std::bind(&RsbpearlConverterApp::DataCallbackGetPcdShm, this, std::placeholders::_1);
...
bool RsbpearlConverterApp::GetParameters(const std::string& config_file)
{
    try
    {
...
void RsbpearlConverterApp::DataCallbackUseDefaultExtrinsic(const uint8_t* data, size_t size)
{
    scan_count_++;
...
void RsbpearlConverterApp::DataCallbackUseUserExtrinsic(const uint8_t* data, size_t size)
{
    scan_count_++;
...
void RsbpearlConverterApp::DataCallbackGetPcdShm(const uint64_t info)
{
    /* open shm */
...
{
    cxxopts::Options options(argv[0], "rsbpearl-converter");
    // clang-format off
    options.add_options()
        ...
        ("n,node_name", "node name for this app", cxxopts::value<std::string>()->default_value("RsbpearlConverter"))
    ...
    std::string log_file_name = holo::GetLogFileAbsolutePath("holo_sensors/rsbpearl_converter_" + node_name + ".log");
    ...
    try
    {
        holo_cmw::RsbpearlConverterApp driver(domain_id, "");
