...
/**
 * @file pandar128_converter.cpp
 * @brief A converter to convert pandar128 scan messages to pcd
 * @author wujie<wujie@holomatic.com>
 * @date 2021/12/30
 */

#include <lidar/pandar128/pandar128_converter_app.h>
#include <csignal>

namespace holo_cmw
{
void Pandar128ConverterApp::Setup(const std::string& config_file, const std::string& out_fmt)
{
    ...
    if (!GetParameters(config_file))
    {
        ...
        oss << "Pandar128 lidar converter app failed to load config file";
        LOG(ERROR) << oss.str();
    ...
    try
    {
        ...
        if (publish_scan_)
        {
            ...
            if (extrinsic_path_ == "")
            {
                h0 = std::bind(&Pandar128ConverterApp::DataCallbackUseDefaultExtrinsic, this, std::placeholders::_1,
                               std::placeholders::_2);
                LOG(INFO) << "use default extrinsics from LidarScan";
            }
            else
            {
                h0 = std::bind(&Pandar128ConverterApp::DataCallbackUseUserExtrinsic, this, std::placeholders::_1,
                               std::placeholders::_2);
        ...
        else
        {
            ...
            h0 = std::bind(&Pandar128ConverterApp::DataCallbackGetPcdShm, this, std::placeholders::_1);
...
bool Pandar128ConverterApp::GetParameters(const std::string& config_file)
{
    try
    {
...
void Pandar128ConverterApp::AddTask(PointCloudType&& pcd)
{
    /* save ply file */
...
void Pandar128ConverterApp::DataCallbackUseDefaultExtrinsic(const uint8_t* data, size_t size)
{
    scan_count_++;
...
void Pandar128ConverterApp::DataCallbackUseUserExtrinsic(const uint8_t* data, size_t size)
{
    scan_count_++;
...
void Pandar128ConverterApp::DataCallbackGetPcdShm(const uint64_t info)
{
    /* open shm */
...
{
    cxxopts::Options options(argv[0], "pandar128-converter");
    // clang-format off
    options.add_options()
        ...
        ("n,node_name", "node name for this app", cxxopts::value<std::string>()->default_value("Pandar128Converter"))
    ...
    std::string log_file_name = holo::GetLogFileAbsolutePath("holo_sensors/pandar128_converter_" + node_name + ".log");
    ...
    try
    {
        holo_cmw::Pandar128ConverterApp driver(domain_id, "");
