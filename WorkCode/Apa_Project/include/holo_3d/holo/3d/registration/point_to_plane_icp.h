...
/**
 * @file point_to_plane_icp.h
 * @brief This header file defines point to plane icp class
 * @author mengzhili(mengzhili@holomaitc.com)
 * @date 2021-01-31
 */

#ifndef HOLO_3D_REGISTRATION_POINT_TO_PLANE_ICP_H_
#define HOLO_3D_REGISTRAITON_POINT_TO_PLANE_ICP_H_
...
class PointToPlaneICPT : public IterativeClosestPointT<PointSourceT, PointTargetT, Scalar>
{
public:
    ...
    /**
     * @brief Construct a new Point To Plane ICP object
     *
     * @param[in] scale_cloud rescale point cloud or not in transformation estimation method
     */
    PointToPlaneICPT(bool_t scale_cloud = false);

    /**
     * @brief Destroy the Point To Point ICP object
     *
     */
    virtual ~PointToPlaneICPT();

    /**
     * @brief Set the maximum angle threshold between two correspondent point normal vector in source and target.
     * correspondence are rejected if the angle between the normals is bigger than this threshold.
     *
     * @param angle the maximum angle between the normals. Set in degrees.
     */
    void SetMaxAngle(Scalar angle);

    /**
     * @brief Get the maximum angle threshold between the normals
     *
     * @return Scalar the maximum angle between the normals in degrees.
     */
    Scalar GetMaxAngle() const noexcept;

    /**
     * @brief transformation computation method point2plane with initial guess
     *
...
private:
    Scalar max_angle_;  ///< max angle between point normal.
...
#endif  // HOLO_3D_REGISTRATION_POINT_TO_PLANE_ICP_H_
