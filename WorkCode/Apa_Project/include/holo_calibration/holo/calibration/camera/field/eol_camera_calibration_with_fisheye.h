...
/**
 * @file eol_fisheye_camera_calibration_with_fisheye.h
 * @brief class for EOL(end of line) fisheye calibration
 * @author jiaxing.zhao @ zhaojiaxing@holomatic.com
 * @date 2021-06-03
 */

#ifndef HOLO_CALIBRATION_CAMERA_FIELD_EOL_CAMERA_CALIBRATION_WITH_FISHEYE_H_
#define HOLO_CALIBRATION_CAMERA_FIELD_EOL_CAMERA_CALIBRATION_WITH_FISHEYE_H_
 ...
 */
class EOLCameraCalibration : public EOLCameraCalibrationBase
{
public:
     ...
     * @param[in] source_image Source images, sufficient number of AprilTag should be contained.
     ...
     */
    EOLCameraCalibration(AprilTagDatabasePtr const& apriltag_db, std::vector<Image> const& source_images,
     ...
     * @throw if the camera model is not FISHEYE, throw
     */
    EOLCameraCalibration(ChessboardDatabasePtr const& chessboard_db, std::vector<Image> const& source_images,
                         CameraModel const& camera_model, Parameters const& parameters);
     ...
     * @throw if the camera model is not FISHEYE, throw
     */
    EOLCameraCalibration(ChArUcobDatabasePtr const& charucob_db, CameraModel const& camera_model,
                         std::vector<Image> const& source_images, Parameters const& parameters);

    /**
     * @brief Destruction
     */
    virtual ~EOLCameraCalibration()
    {
    }

    /**
     * @brief Set the April Tag Database object
     *
     * @param[in] apriltag_db apriltag data base
     */
    void SetAprilTagDatabase(AprilTagDatabasePtr const& apriltag_db)
    {
        apriltag_db_ = apriltag_db;
        tag_type_    = TagType::AprilTag;
    }
...
protected:
    ...
    /**
     * @brief Get the Correspondence By April Tag object
     *
     * @param gray_image
     * @param rgb_image
     * @param apriltag_db
     ...
     * @return a set of 2D-3D corresponding points.
     */
    Correspondence getCorrespondenceByAprilTag(cv::Mat const& gray_image, cv::Mat const& rgb_image,
                                               AprilTagDatabasePtr const& apriltag_db, Parameters const& params,
                                               TagType const& tag_type);
...
protected:
     ...
     *
     */
    AprilTagDatabasePtr apriltag_db_;
