// -*- C++ -*-
// $Id$

/**
 * Code generated by the The ACE ORB (TAO) IDL Compiler v2.2a_p19
 * TAO and the TAO IDL Compiler have been developed by:
 *       Center for Distributed Object Computing
 *       Washington University
 *       St. Louis, MO
 *       USA
 *       http://www.cs.wustl.edu/~schmidt/doc-center.html
 * and
 *       Distributed Object Computing Laboratory
 *       University of California at Irvine
 *       Irvine, CA
 *       USA
 * and
 *       Institute for Software Integrated Systems
 *       Vanderbilt University
 *       Nashville, TN
 *       USA
 *       http://www.isis.vanderbilt.edu/
 *
 * Information about TAO is available at:
 *     http://www.cs.wustl.edu/~schmidt/TAO.html
 **/

// TAO_IDL - Generated from
// be/be_codegen.cpp:152

#ifndef _TAO_IDL__HOME_YUEPF_DOCUMENTS_CODE_APA_PROJECT_BUILD_TDA4_UBUNTU_18_04_DEBUG_HOLO_PARKING_MSG_INCLUDE_HOLO_PARKING_MSG_CONTROL_APACONTROLCOMMANDTYPESUPPORTC_AJSDU1_H_
#define _TAO_IDL__HOME_YUEPF_DOCUMENTS_CODE_APA_PROJECT_BUILD_TDA4_UBUNTU_18_04_DEBUG_HOLO_PARKING_MSG_INCLUDE_HOLO_PARKING_MSG_CONTROL_APACONTROLCOMMANDTYPESUPPORTC_AJSDU1_H_

#include /**/ "ace/pre.h"


#include /**/ "ace/config-all.h"

#if !defined (ACE_LACKS_PRAGMA_ONCE)
# pragma once
#endif /* ACE_LACKS_PRAGMA_ONCE */


#include "tao/AnyTypeCode/AnyTypeCode_methods.h"
#include "tao/AnyTypeCode/Any.h"
#include "tao/ORB.h"
#include "tao/SystemException.h"
#include "tao/Basic_Types.h"
#include "tao/ORB_Constants.h"
#include "dds/DCPS/ZeroCopyInfoSeq_T.h"
#include "dds/DCPS/ZeroCopySeq_T.h"
#include "tao/Object.h"
#include "tao/String_Manager_T.h"
#include "tao/Sequence_T.h"
#include "tao/Objref_VarOut_T.h"
#include "tao/Seq_Var_T.h"
#include "tao/Seq_Out_T.h"
#include "tao/Arg_Traits_T.h"
#include "tao/Basic_Arguments.h"
#include "tao/Special_Basic_Arguments.h"
#include "tao/Any_Insert_Policy_T.h"
#include "tao/Fixed_Size_Argument_T.h"
#include "tao/Var_Size_Argument_T.h"
#include /**/ "tao/Version.h"
#include /**/ "tao/Versioned_Namespace.h"

#include "ApaControlCommandC.h"
#include "dds/DdsDcpsInfrastructureC.h"
#include "dds/DdsDcpsPublicationC.h"
#include "dds/DdsDcpsSubscriptionExtC.h"
#include "dds/DdsDcpsTopicC.h"
#include "dds/DdsDcpsTypeSupportExtC.h"

#if TAO_MAJOR_VERSION != 2 || TAO_MINOR_VERSION != 2 || TAO_BETA_VERSION != 0
#error This file should be regenerated with TAO_IDL
#endif

#if defined (TAO_EXPORT_MACRO)
#undef TAO_EXPORT_MACRO
#endif
#define TAO_EXPORT_MACRO 

// TAO_IDL - Generated from
// be/be_visitor_module/module_ch.cpp:38

namespace holo_parking_msg
{

  // TAO_IDL - Generated from
  // be/be_visitor_module/module_ch.cpp:38

  namespace control
  {

    // TAO_IDL - Generated from
    // be/be_visitor_sequence/sequence_ch.cpp:102

    typedef ::TAO::DCPS::ZeroCopyDataSeq< holo_parking_msg::control::ApaControlCommand, DCPS_ZERO_COPY_SEQ_DEFAULT_SIZE> ApaControlCommandSeq;
    

    // TAO_IDL - Generated from
    // be/be_visitor_typecode/typecode_decl.cpp:37

    extern  ::CORBA::TypeCode_ptr const _tc_ApaControlCommandSeq;

    // TAO_IDL - Generated from
    // be/be_interface.cpp:751

#if !defined (_HOLO_PARKING_MSG_CONTROL_APACONTROLCOMMANDTYPESUPPORT__VAR_OUT_CH_)
#define _HOLO_PARKING_MSG_CONTROL_APACONTROLCOMMANDTYPESUPPORT__VAR_OUT_CH_

    class ApaControlCommandTypeSupport;
    typedef ApaControlCommandTypeSupport *ApaControlCommandTypeSupport_ptr;

    typedef
      TAO_Objref_Var_T<
          ApaControlCommandTypeSupport
        >
      ApaControlCommandTypeSupport_var;
    
    typedef
      TAO_Objref_Out_T<
          ApaControlCommandTypeSupport
        >
      ApaControlCommandTypeSupport_out;

#endif /* end #if !defined */

    // TAO_IDL - Generated from
    // be/be_visitor_interface/interface_ch.cpp:43

    class  ApaControlCommandTypeSupport
      : public virtual ::OpenDDS::DCPS::TypeSupport
    
    {
    public:

      // TAO_IDL - Generated from
      // be/be_type.cpp:307

      typedef ApaControlCommandTypeSupport_ptr _ptr_type;
      typedef ApaControlCommandTypeSupport_var _var_type;
      typedef ApaControlCommandTypeSupport_out _out_type;

      static void _tao_any_destructor (void *);

      // The static operations.
      static ApaControlCommandTypeSupport_ptr _duplicate (ApaControlCommandTypeSupport_ptr obj);

      static void _tao_release (ApaControlCommandTypeSupport_ptr obj);

      static ApaControlCommandTypeSupport_ptr _narrow (::CORBA::Object_ptr obj);
      static ApaControlCommandTypeSupport_ptr _unchecked_narrow (::CORBA::Object_ptr obj);
      static ApaControlCommandTypeSupport_ptr _nil (void);

      // TAO_IDL - Generated from
      // be/be_visitor_interface/interface_ch.cpp:140

      virtual ::CORBA::Boolean _is_a (const char *type_id);
      virtual const char* _interface_repository_id (void) const;
      virtual ::CORBA::Boolean marshal (TAO_OutputCDR &cdr);
    
    protected:
      // Abstract or local interface only.
      ApaControlCommandTypeSupport (void);

      

      virtual ~ApaControlCommandTypeSupport (void);
    
    private:
      // Private and unimplemented for concrete interfaces.
      ACE_UNIMPLEMENTED_FUNC (ApaControlCommandTypeSupport (const ApaControlCommandTypeSupport &))
#if defined (ACE_HAS_CPP11)
      ACE_UNIMPLEMENTED_FUNC (ApaControlCommandTypeSupport (ApaControlCommandTypeSupport &&))
#endif /* ACE_HAS_CPP11 */
      ACE_UNIMPLEMENTED_FUNC (ApaControlCommandTypeSupport &operator= (const ApaControlCommandTypeSupport &))
#if defined (ACE_HAS_CPP11)
      ACE_UNIMPLEMENTED_FUNC (ApaControlCommandTypeSupport &operator= (ApaControlCommandTypeSupport &&))
#endif /* ACE_HAS_CPP11 */
    };

    // TAO_IDL - Generated from
    // be/be_visitor_typecode/typecode_decl.cpp:37

    extern  ::CORBA::TypeCode_ptr const _tc_ApaControlCommandTypeSupport;

    // TAO_IDL - Generated from
    // be/be_interface.cpp:751

#if !defined (_HOLO_PARKING_MSG_CONTROL_APACONTROLCOMMANDDATAWRITER__VAR_OUT_CH_)
#define _HOLO_PARKING_MSG_CONTROL_APACONTROLCOMMANDDATAWRITER__VAR_OUT_CH_

    class ApaControlCommandDataWriter;
    typedef ApaControlCommandDataWriter *ApaControlCommandDataWriter_ptr;

    typedef
      TAO_Objref_Var_T<
          ApaControlCommandDataWriter
        >
      ApaControlCommandDataWriter_var;
    
    typedef
      TAO_Objref_Out_T<
          ApaControlCommandDataWriter
        >
      ApaControlCommandDataWriter_out;

#endif /* end #if !defined */

    // TAO_IDL - Generated from
    // be/be_visitor_interface/interface_ch.cpp:43

    class  ApaControlCommandDataWriter
      : public virtual ::DDS::DataWriter
    
    {
    public:

      // TAO_IDL - Generated from
      // be/be_type.cpp:307

      typedef ApaControlCommandDataWriter_ptr _ptr_type;
      typedef ApaControlCommandDataWriter_var _var_type;
      typedef ApaControlCommandDataWriter_out _out_type;

      static void _tao_any_destructor (void *);

      // The static operations.
      static ApaControlCommandDataWriter_ptr _duplicate (ApaControlCommandDataWriter_ptr obj);

      static void _tao_release (ApaControlCommandDataWriter_ptr obj);

      static ApaControlCommandDataWriter_ptr _narrow (::CORBA::Object_ptr obj);
      static ApaControlCommandDataWriter_ptr _unchecked_narrow (::CORBA::Object_ptr obj);
      static ApaControlCommandDataWriter_ptr _nil (void);

      virtual ::DDS::InstanceHandle_t register_instance (
        const ::holo_parking_msg::control::ApaControlCommand & instance) = 0;

      virtual ::DDS::InstanceHandle_t register_instance_w_timestamp (
        const ::holo_parking_msg::control::ApaControlCommand & instance,
        const ::DDS::Time_t & timestamp) = 0;

      virtual ::DDS::ReturnCode_t unregister_instance (
        const ::holo_parking_msg::control::ApaControlCommand & instance,
        ::DDS::InstanceHandle_t handle) = 0;

      virtual ::DDS::ReturnCode_t unregister_instance_w_timestamp (
        const ::holo_parking_msg::control::ApaControlCommand & instance,
        ::DDS::InstanceHandle_t handle,
        const ::DDS::Time_t & timestamp) = 0;

      virtual ::DDS::ReturnCode_t write (
        const ::holo_parking_msg::control::ApaControlCommand & instance_data,
        ::DDS::InstanceHandle_t handle) = 0;

      virtual ::DDS::ReturnCode_t write_w_timestamp (
        const ::holo_parking_msg::control::ApaControlCommand & instance_data,
        ::DDS::InstanceHandle_t handle,
        const ::DDS::Time_t & source_timestamp) = 0;

      virtual ::DDS::ReturnCode_t dispose (
        const ::holo_parking_msg::control::ApaControlCommand & instance_data,
        ::DDS::InstanceHandle_t instance_handle) = 0;

      virtual ::DDS::ReturnCode_t dispose_w_timestamp (
        const ::holo_parking_msg::control::ApaControlCommand & instance_data,
        ::DDS::InstanceHandle_t instance_handle,
        const ::DDS::Time_t & source_timestamp) = 0;

      virtual ::DDS::ReturnCode_t get_key_value (
        ::holo_parking_msg::control::ApaControlCommand & key_holder,
        ::DDS::InstanceHandle_t handle) = 0;

      virtual ::DDS::InstanceHandle_t lookup_instance (
        const ::holo_parking_msg::control::ApaControlCommand & instance_data) = 0;

      // TAO_IDL - Generated from
      // be/be_visitor_interface/interface_ch.cpp:140

      virtual ::CORBA::Boolean _is_a (const char *type_id);
      virtual const char* _interface_repository_id (void) const;
      virtual ::CORBA::Boolean marshal (TAO_OutputCDR &cdr);
    
    protected:
      // Abstract or local interface only.
      ApaControlCommandDataWriter (void);

      

      virtual ~ApaControlCommandDataWriter (void);
    
    private:
      // Private and unimplemented for concrete interfaces.
      ACE_UNIMPLEMENTED_FUNC (ApaControlCommandDataWriter (const ApaControlCommandDataWriter &))
#if defined (ACE_HAS_CPP11)
      ACE_UNIMPLEMENTED_FUNC (ApaControlCommandDataWriter (ApaControlCommandDataWriter &&))
#endif /* ACE_HAS_CPP11 */
      ACE_UNIMPLEMENTED_FUNC (ApaControlCommandDataWriter &operator= (const ApaControlCommandDataWriter &))
#if defined (ACE_HAS_CPP11)
      ACE_UNIMPLEMENTED_FUNC (ApaControlCommandDataWriter &operator= (ApaControlCommandDataWriter &&))
#endif /* ACE_HAS_CPP11 */
    };

    // TAO_IDL - Generated from
    // be/be_visitor_typecode/typecode_decl.cpp:37

    extern  ::CORBA::TypeCode_ptr const _tc_ApaControlCommandDataWriter;

    // TAO_IDL - Generated from
    // be/be_interface.cpp:751

#if !defined (_HOLO_PARKING_MSG_CONTROL_APACONTROLCOMMANDDATAREADER__VAR_OUT_CH_)
#define _HOLO_PARKING_MSG_CONTROL_APACONTROLCOMMANDDATAREADER__VAR_OUT_CH_

    class ApaControlCommandDataReader;
    typedef ApaControlCommandDataReader *ApaControlCommandDataReader_ptr;

    typedef
      TAO_Objref_Var_T<
          ApaControlCommandDataReader
        >
      ApaControlCommandDataReader_var;
    
    typedef
      TAO_Objref_Out_T<
          ApaControlCommandDataReader
        >
      ApaControlCommandDataReader_out;

#endif /* end #if !defined */

    // TAO_IDL - Generated from
    // be/be_visitor_interface/interface_ch.cpp:43

    class  ApaControlCommandDataReader
      : public virtual ::OpenDDS::DCPS::DataReaderEx
    
    {
    public:

      // TAO_IDL - Generated from
      // be/be_type.cpp:307

      typedef ApaControlCommandDataReader_ptr _ptr_type;
      typedef ApaControlCommandDataReader_var _var_type;
      typedef ApaControlCommandDataReader_out _out_type;

      static void _tao_any_destructor (void *);

      // The static operations.
      static ApaControlCommandDataReader_ptr _duplicate (ApaControlCommandDataReader_ptr obj);

      static void _tao_release (ApaControlCommandDataReader_ptr obj);

      static ApaControlCommandDataReader_ptr _narrow (::CORBA::Object_ptr obj);
      static ApaControlCommandDataReader_ptr _unchecked_narrow (::CORBA::Object_ptr obj);
      static ApaControlCommandDataReader_ptr _nil (void);

      virtual ::DDS::ReturnCode_t read (
        ::holo_parking_msg::control::ApaControlCommandSeq & received_data,
        ::DDS::SampleInfoSeq & info_seq,
        ::CORBA::Long max_samples,
        ::DDS::SampleStateMask sample_states,
        ::DDS::ViewStateMask view_states,
        ::DDS::InstanceStateMask instance_states) = 0;

      virtual ::DDS::ReturnCode_t take (
        ::holo_parking_msg::control::ApaControlCommandSeq & received_data,
        ::DDS::SampleInfoSeq & info_seq,
        ::CORBA::Long max_samples,
        ::DDS::SampleStateMask sample_states,
        ::DDS::ViewStateMask view_states,
        ::DDS::InstanceStateMask instance_states) = 0;

      virtual ::DDS::ReturnCode_t read_w_condition (
        ::holo_parking_msg::control::ApaControlCommandSeq & data_values,
        ::DDS::SampleInfoSeq & sample_infos,
        ::CORBA::Long max_samples,
        ::DDS::ReadCondition_ptr a_condition) = 0;

      virtual ::DDS::ReturnCode_t take_w_condition (
        ::holo_parking_msg::control::ApaControlCommandSeq & data_values,
        ::DDS::SampleInfoSeq & sample_infos,
        ::CORBA::Long max_samples,
        ::DDS::ReadCondition_ptr a_condition) = 0;

      virtual ::DDS::ReturnCode_t read_next_sample (
        ::holo_parking_msg::control::ApaControlCommand & received_data,
        ::DDS::SampleInfo & sample_info) = 0;

      virtual ::DDS::ReturnCode_t take_next_sample (
        ::holo_parking_msg::control::ApaControlCommand & received_data,
        ::DDS::SampleInfo & sample_info) = 0;

      virtual ::DDS::ReturnCode_t read_instance (
        ::holo_parking_msg::control::ApaControlCommandSeq & received_data,
        ::DDS::SampleInfoSeq & info_seq,
        ::CORBA::Long max_samples,
        ::DDS::InstanceHandle_t a_handle,
        ::DDS::SampleStateMask sample_states,
        ::DDS::ViewStateMask view_states,
        ::DDS::InstanceStateMask instance_states) = 0;

      virtual ::DDS::ReturnCode_t take_instance (
        ::holo_parking_msg::control::ApaControlCommandSeq & received_data,
        ::DDS::SampleInfoSeq & info_seq,
        ::CORBA::Long max_samples,
        ::DDS::InstanceHandle_t a_handle,
        ::DDS::SampleStateMask sample_states,
        ::DDS::ViewStateMask view_states,
        ::DDS::InstanceStateMask instance_states) = 0;

      virtual ::DDS::ReturnCode_t read_instance_w_condition (
        ::holo_parking_msg::control::ApaControlCommandSeq & received_data,
        ::DDS::SampleInfoSeq & sample_infos,
        ::CORBA::Long max_samples,
        ::DDS::InstanceHandle_t a_handle,
        ::DDS::ReadCondition_ptr a_condition) = 0;

      virtual ::DDS::ReturnCode_t take_instance_w_condition (
        ::holo_parking_msg::control::ApaControlCommandSeq & received_data,
        ::DDS::SampleInfoSeq & sample_infos,
        ::CORBA::Long max_samples,
        ::DDS::InstanceHandle_t a_handle,
        ::DDS::ReadCondition_ptr a_condition) = 0;

      virtual ::DDS::ReturnCode_t read_next_instance (
        ::holo_parking_msg::control::ApaControlCommandSeq & received_data,
        ::DDS::SampleInfoSeq & info_seq,
        ::CORBA::Long max_samples,
        ::DDS::InstanceHandle_t a_handle,
        ::DDS::SampleStateMask sample_states,
        ::DDS::ViewStateMask view_states,
        ::DDS::InstanceStateMask instance_states) = 0;

      virtual ::DDS::ReturnCode_t take_next_instance (
        ::holo_parking_msg::control::ApaControlCommandSeq & received_data,
        ::DDS::SampleInfoSeq & info_seq,
        ::CORBA::Long max_samples,
        ::DDS::InstanceHandle_t a_handle,
        ::DDS::SampleStateMask sample_states,
        ::DDS::ViewStateMask view_states,
        ::DDS::InstanceStateMask instance_states) = 0;

      virtual ::DDS::ReturnCode_t read_next_instance_w_condition (
        ::holo_parking_msg::control::ApaControlCommandSeq & data_values,
        ::DDS::SampleInfoSeq & sample_infos,
        ::CORBA::Long max_samples,
        ::DDS::InstanceHandle_t previous_handle,
        ::DDS::ReadCondition_ptr a_condition) = 0;

      virtual ::DDS::ReturnCode_t take_next_instance_w_condition (
        ::holo_parking_msg::control::ApaControlCommandSeq & data_values,
        ::DDS::SampleInfoSeq & sample_infos,
        ::CORBA::Long max_samples,
        ::DDS::InstanceHandle_t previous_handle,
        ::DDS::ReadCondition_ptr a_condition) = 0;

      virtual ::DDS::ReturnCode_t return_loan (
        ::holo_parking_msg::control::ApaControlCommandSeq & received_data,
        ::DDS::SampleInfoSeq & info_seq) = 0;

      virtual ::DDS::ReturnCode_t get_key_value (
        ::holo_parking_msg::control::ApaControlCommand & key_holder,
        ::DDS::InstanceHandle_t handle) = 0;

      virtual ::DDS::InstanceHandle_t lookup_instance (
        const ::holo_parking_msg::control::ApaControlCommand & instance_data) = 0;

      // TAO_IDL - Generated from
      // be/be_visitor_interface/interface_ch.cpp:140

      virtual ::CORBA::Boolean _is_a (const char *type_id);
      virtual const char* _interface_repository_id (void) const;
      virtual ::CORBA::Boolean marshal (TAO_OutputCDR &cdr);
    
    protected:
      // Abstract or local interface only.
      ApaControlCommandDataReader (void);

      

      virtual ~ApaControlCommandDataReader (void);
    
    private:
      // Private and unimplemented for concrete interfaces.
      ACE_UNIMPLEMENTED_FUNC (ApaControlCommandDataReader (const ApaControlCommandDataReader &))
#if defined (ACE_HAS_CPP11)
      ACE_UNIMPLEMENTED_FUNC (ApaControlCommandDataReader (ApaControlCommandDataReader &&))
#endif /* ACE_HAS_CPP11 */
      ACE_UNIMPLEMENTED_FUNC (ApaControlCommandDataReader &operator= (const ApaControlCommandDataReader &))
#if defined (ACE_HAS_CPP11)
      ACE_UNIMPLEMENTED_FUNC (ApaControlCommandDataReader &operator= (ApaControlCommandDataReader &&))
#endif /* ACE_HAS_CPP11 */
    };

    // TAO_IDL - Generated from
    // be/be_visitor_typecode/typecode_decl.cpp:37

    extern  ::CORBA::TypeCode_ptr const _tc_ApaControlCommandDataReader;
  
  // TAO_IDL - Generated from
  // be/be_visitor_module/module_ch.cpp:67
  
  } // module holo_parking_msg::control

// TAO_IDL - Generated from
// be/be_visitor_module/module_ch.cpp:67

} // module holo_parking_msg

// TAO_IDL - Generated from
// be/be_visitor_arg_traits.cpp:68

TAO_BEGIN_VERSIONED_NAMESPACE_DECL


// Arg traits specializations.
namespace TAO
{
}

TAO_END_VERSIONED_NAMESPACE_DECL



// TAO_IDL - Generated from
// be/be_visitor_traits.cpp:62

TAO_BEGIN_VERSIONED_NAMESPACE_DECL

// Traits specializations.
namespace TAO
{

#if !defined (_HOLO_PARKING_MSG_CONTROL_APACONTROLCOMMANDTYPESUPPORT__TRAITS_)
#define _HOLO_PARKING_MSG_CONTROL_APACONTROLCOMMANDTYPESUPPORT__TRAITS_

  template<>
  struct  Objref_Traits< ::holo_parking_msg::control::ApaControlCommandTypeSupport>
  {
    static ::holo_parking_msg::control::ApaControlCommandTypeSupport_ptr duplicate (
        ::holo_parking_msg::control::ApaControlCommandTypeSupport_ptr p);
    static void release (
        ::holo_parking_msg::control::ApaControlCommandTypeSupport_ptr p);
    static ::holo_parking_msg::control::ApaControlCommandTypeSupport_ptr nil (void);
    static ::CORBA::Boolean marshal (
        const ::holo_parking_msg::control::ApaControlCommandTypeSupport_ptr p,
        TAO_OutputCDR & cdr);
  };

#endif /* end #if !defined */

#if !defined (_HOLO_PARKING_MSG_CONTROL_APACONTROLCOMMANDDATAWRITER__TRAITS_)
#define _HOLO_PARKING_MSG_CONTROL_APACONTROLCOMMANDDATAWRITER__TRAITS_

  template<>
  struct  Objref_Traits< ::holo_parking_msg::control::ApaControlCommandDataWriter>
  {
    static ::holo_parking_msg::control::ApaControlCommandDataWriter_ptr duplicate (
        ::holo_parking_msg::control::ApaControlCommandDataWriter_ptr p);
    static void release (
        ::holo_parking_msg::control::ApaControlCommandDataWriter_ptr p);
    static ::holo_parking_msg::control::ApaControlCommandDataWriter_ptr nil (void);
    static ::CORBA::Boolean marshal (
        const ::holo_parking_msg::control::ApaControlCommandDataWriter_ptr p,
        TAO_OutputCDR & cdr);
  };

#endif /* end #if !defined */

#if !defined (_HOLO_PARKING_MSG_CONTROL_APACONTROLCOMMANDDATAREADER__TRAITS_)
#define _HOLO_PARKING_MSG_CONTROL_APACONTROLCOMMANDDATAREADER__TRAITS_

  template<>
  struct  Objref_Traits< ::holo_parking_msg::control::ApaControlCommandDataReader>
  {
    static ::holo_parking_msg::control::ApaControlCommandDataReader_ptr duplicate (
        ::holo_parking_msg::control::ApaControlCommandDataReader_ptr p);
    static void release (
        ::holo_parking_msg::control::ApaControlCommandDataReader_ptr p);
    static ::holo_parking_msg::control::ApaControlCommandDataReader_ptr nil (void);
    static ::CORBA::Boolean marshal (
        const ::holo_parking_msg::control::ApaControlCommandDataReader_ptr p,
        TAO_OutputCDR & cdr);
  };

#endif /* end #if !defined */
}
TAO_END_VERSIONED_NAMESPACE_DECL



// TAO_IDL - Generated from
// be/be_visitor_interface/any_op_ch.cpp:44



#if defined (ACE_ANY_OPS_USE_NAMESPACE)

namespace holo_parking_msg
{
  
  namespace control
  {
     void operator<<= ( ::CORBA::Any &, ApaControlCommandTypeSupport_ptr); // copying
     void operator<<= ( ::CORBA::Any &, ApaControlCommandTypeSupport_ptr *); // non-copying
     ::CORBA::Boolean operator>>= (const ::CORBA::Any &, ApaControlCommandTypeSupport_ptr &);
  }
}

#else


TAO_BEGIN_VERSIONED_NAMESPACE_DECL

 void operator<<= (::CORBA::Any &, holo_parking_msg::control::ApaControlCommandTypeSupport_ptr); // copying
 void operator<<= (::CORBA::Any &, holo_parking_msg::control::ApaControlCommandTypeSupport_ptr *); // non-copying
 ::CORBA::Boolean operator>>= (const ::CORBA::Any &, holo_parking_msg::control::ApaControlCommandTypeSupport_ptr &);
TAO_END_VERSIONED_NAMESPACE_DECL



#endif

// TAO_IDL - Generated from
// be/be_visitor_interface/any_op_ch.cpp:44



#if defined (ACE_ANY_OPS_USE_NAMESPACE)

namespace holo_parking_msg
{
  
  namespace control
  {
     void operator<<= ( ::CORBA::Any &, ApaControlCommandDataWriter_ptr); // copying
     void operator<<= ( ::CORBA::Any &, ApaControlCommandDataWriter_ptr *); // non-copying
     ::CORBA::Boolean operator>>= (const ::CORBA::Any &, ApaControlCommandDataWriter_ptr &);
  }
}

#else


TAO_BEGIN_VERSIONED_NAMESPACE_DECL

 void operator<<= (::CORBA::Any &, holo_parking_msg::control::ApaControlCommandDataWriter_ptr); // copying
 void operator<<= (::CORBA::Any &, holo_parking_msg::control::ApaControlCommandDataWriter_ptr *); // non-copying
 ::CORBA::Boolean operator>>= (const ::CORBA::Any &, holo_parking_msg::control::ApaControlCommandDataWriter_ptr &);
TAO_END_VERSIONED_NAMESPACE_DECL



#endif

// TAO_IDL - Generated from
// be/be_visitor_interface/any_op_ch.cpp:44



#if defined (ACE_ANY_OPS_USE_NAMESPACE)

namespace holo_parking_msg
{
  
  namespace control
  {
     void operator<<= ( ::CORBA::Any &, ApaControlCommandDataReader_ptr); // copying
     void operator<<= ( ::CORBA::Any &, ApaControlCommandDataReader_ptr *); // non-copying
     ::CORBA::Boolean operator>>= (const ::CORBA::Any &, ApaControlCommandDataReader_ptr &);
  }
}

#else


TAO_BEGIN_VERSIONED_NAMESPACE_DECL

 void operator<<= (::CORBA::Any &, holo_parking_msg::control::ApaControlCommandDataReader_ptr); // copying
 void operator<<= (::CORBA::Any &, holo_parking_msg::control::ApaControlCommandDataReader_ptr *); // non-copying
 ::CORBA::Boolean operator>>= (const ::CORBA::Any &, holo_parking_msg::control::ApaControlCommandDataReader_ptr &);
TAO_END_VERSIONED_NAMESPACE_DECL



#endif

// TAO_IDL - Generated from
// be/be_codegen.cpp:1703
#if defined (__ACE_INLINE__)
#include "ApaControlCommandTypeSupportC.inl"
#endif /* defined INLINE */

#include /**/ "ace/post.h"

#endif /* ifndef */

