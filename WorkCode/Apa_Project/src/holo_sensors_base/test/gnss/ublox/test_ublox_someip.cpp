...
TEST(UbloxDriverSomeip, ParseData1)
{
    UbloxDriverT<SomeipPacketType>   driver(0);
...
TEST(UbloxDriverSomeip, ParseData2)
{
    UbloxDriverT<SomeipPacketType>   driver(0);
    holo::common::GnssRawMeasurement raw;
    bool                             called1 = false;
    ...
    SomeipPacketType packet;
    packet.SetMsgLength(8 + 4 + sizeof(rxm_rawx_msg));
    packet.SetPayload(4, rxm_rawx_msg, sizeof(rxm_rawx_msg));
    driver.ParseData(packet);
    ASSERT_TRUE(called1);
    packet.SetMsgLength(8 + 4 + sizeof(nav_pvt_msg));
    packet.SetPayload(4, nav_pvt_msg, sizeof(nav_pvt_msg));
    driver.ParseData(packet);
    ASSERT_TRUE(called2);
}
