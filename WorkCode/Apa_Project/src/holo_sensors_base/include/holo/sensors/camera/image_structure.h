...
using TimestampPair = std::pair<Timestamp, Timestamp>;
...
struct ImagePacket
{
    ...
    bool_t Deserialize(D& deserializer)
    {
        ...
        Timestamp _recv_stamp, _trigger_stamp;
        try
        {
            deserializer >> _seq;
    ...
    uint32_t                        seq;
    mutable holo::common::Timestamp received_stamp;
    mutable holo::common::Timestamp trigger_stamp;
    ImageType                       image_type;
    uint8_t                         reserved[2];
    uint8_t                         frame_type;
    mutable uint32_t                cols;
    mutable uint32_t                rows;
    uint32_t                        size;
    uint8_t*                        data;

    std::function<void(uint8_t**)> deallocator;
};
