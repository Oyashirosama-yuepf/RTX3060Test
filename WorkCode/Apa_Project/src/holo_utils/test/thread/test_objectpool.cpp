...
int main(int argc, char** argv)
{
    ...
    // ObjectPool<std::function<void()>>                  object_pool{1024};
    std::function<void()> const                        aaaa = std::bind(test_task2, 888, 555.679092);
    std::shared_ptr<ObjectPool<std::function<void()>>> object_pool =
        std::make_shared<ObjectPool<std::function<void()>>>();

    std::weak_ptr<ObjectPool<std::function<void()>>> weak_pool = object_pool;
    ...
    std::cout << "  shared_ptr use count:" << object_pool.use_count() << std::endl;
    if (1)
    {
        std::shared_ptr<std::function<void()>> f_test_task1 = object_pool->Get();
        std::cout << "use count:" << object_pool->GetUsedSize() << std::endl;
        std::cout << "  shared_ptr use count:" << object_pool.use_count() << std::endl;
        std::shared_ptr<std::function<void()>> f_test_task2 = object_pool->Get();
        std::cout << "use count:" << object_pool->GetUsedSize() << std::endl;
        std::cout << "  shared_ptr use count:" << object_pool.use_count() << std::endl;
        std::shared_ptr<std::function<void()>> f_test_task3 = object_pool->Get();
        std::cout << "use count:" << object_pool->GetUsedSize() << std::endl;
        std::cout << "  shared_ptr use count:" << object_pool.use_count() << std::endl;
        std::shared_ptr<std::function<void()>> f_test_task4 = object_pool->Get();
        std::cout << "use count:" << object_pool->GetUsedSize() << std::endl;
        std::cout << "  shared_ptr use count:" << object_pool.use_count() << std::endl;

        object_pool = nullptr;
        std::cout << "  shared_ptr use count:" << weak_pool.use_count() << std::endl;

        *f_test_task1 = std::bind(test_task1);
        *f_test_task2 = std::bind(test_task2, 888, 555.679092);
        *f_test_task3 = std::bind(test_task3, "object pool test task 3");
        *f_test_task4 = std::bind(test_task4, 3333322, "doooppppppppppppppppppppp");
