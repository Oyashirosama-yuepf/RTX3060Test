 ...
 * @brief A converter to convert livox scan messages to pcd
 * @author guoguangchao<guoguangchao@holomatic.com>
 * @date 2021/03/22
 */

#include <holo/sensors/utils/livox_crc.h>
#include <lidar/livox/livox_converter_app.h>
#include <csignal>

namespace holo_cmw
{
template <typename PacketType>
void LivoxConverterApp<PacketType>::Setup(const std::string& config_file, const std::string& out_fmt)
{
    ...
    if (!GetParameters(config_file))
    {
        ...
        oss << "livox converter app failed to load config file";
        LOG(ERROR) << oss.str();
    ...
    try
    {
        if (!use_lidarscan_ || extrinsic_path_ != "")
        {
        ...
        if (publish_scan_)
        {
            ...
            else
            {
                ...
                lidar_scan_ = std::make_shared<LidarScan>(LidarScan::LidarType::LIVOX, use_compress_, extrinsic_, true,
            ...
            if (extrinsic_path_ == "")
            {
                h0 = std::bind(&LivoxConverterApp<PacketType>::DataCallbackUseDefaultExtrinsic, this,
                               std::placeholders::_1, std::placeholders::_2);
                LOG(INFO) << "use default extrinsics from LidarScan";
            }
            else
            {
                h0 = std::bind(&LivoxConverterApp<PacketType>::DataCallbackUseUserExtrinsic, this,
                               std::placeholders::_1, std::placeholders::_2);
        ...
        else
        {
            ...
            h0 = std::bind(&LivoxConverterApp<PacketType>::DataCallbackGetPcdShm, this, std::placeholders::_1);
...
template <typename PacketType>
void LivoxConverterApp<PacketType>::DataCallbackUseDefaultExtrinsic(const uint8_t* data, size_t size)
{
    PointCloudType pcd;
...
template <typename PacketType>
void LivoxConverterApp<PacketType>::DataCallbackUseUserExtrinsic(const uint8_t* data, size_t size)
{
    PointCloudType pcd;
...
template <typename PacketType>
void LivoxConverterApp<PacketType>::DataCallbackGetPcdShm(const uint64_t info)
{
    /* open shm */
...
{
    cxxopts::Options options(argv[0], "livox-converter");
    // clang-format off
    options.add_options()
        ...
        ("n,node_name", "node name for this app", cxxopts::value<std::string>()->default_value("LivoxConverter"))
    ...
    std::string log_file_name = holo::GetLogFileAbsolutePath("holo_sensors/livox_converter_" + node_name + ".log");
    ...
    try
    {
        if ("LivoxConverter" == node_name)
        {
            LOG(INFO) << "Lidar Type: Livox";
            using PacketType = holo::sensors::format::LivoxPacket<96>;
            holo_cmw::LivoxConverterApp<PacketType> driver(domain_id, "");
            driver.Setup(options["config_file"].as<std::string>(), options["output_name"].as<std::string>());
        }
        else if ("LivoxHapConverter" == node_name)
        {
            LOG(INFO) << "Lidar Type: LivoxHap";
            using PacketType = holo::sensors::format::LivoxHapPacket<1440>;
            holo_cmw::LivoxConverterApp<PacketType> driver(domain_id, "");
            driver.Setup(options["config_file"].as<std::string>(), options["output_name"].as<std::string>());
        }
    }
